...
for file_id in range(0, int(1e30)):
    ...
    muon_kinematic_vectors, muon_aux_values = muGAN.generate(
        size=int(1e6), tuned_aux=True
    )
    ...
    np.save(
        "/mnt/storage/scratch/am13743/AUX_GANs_output/muons_and_aux_%d" % random_id,
        combined_array,
    )
