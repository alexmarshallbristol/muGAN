...
seed_auxiliary_distributions = np.load(
    "SHiP_GAN_module/data_files/Seed_auxiliary_values_for_enhanced_generation.npy"
)
...
seed_auxiliary_distributions[where_array, 2] = (
    seed_auxiliary_distributions[where_array, 2] * 1.4
)
...
where_array = where_array[0][: int(shape_i)]
seed_auxiliary_distributions[where_array, 2] = (
    seed_auxiliary_distributions[where_array, 2] * 1.3
)
...
boosted_muon_kinematic_vectors = muGAN.generate_enhanced(
    auxiliary_distributions=seed_auxiliary_distributions, size=int(5e5)
)


""" Plot the output. """
muGAN.plot_kinematics(
    data=boosted_muon_kinematic_vectors,
    filename="MSO_output/MSO_kinematics.png",
...
""" Save to ROOT file. """
muGAN.save_to_ROOT(
    data=boosted_muon_kinematic_vectors, filename="MSO_output/MSO_muons.root"
)
