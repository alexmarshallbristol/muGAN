...
for i in range(0, np.shape(vec_x)[0]):
    for j in range(0, np.shape(vec_y)[0]):
        ...
        muon_kinematic_vectors = np.append(
            muon_kinematic_vectors,
            [[13, vec_x[i], vec_y[j], -7084.5, 0, 0, 100]],
            axis=0,
        )
...
for i in range(0, np.shape(vec_px)[0]):
    for j in range(0, np.shape(vec_py)[0]):
        ...
        muon_kinematic_vectors = np.append(
            muon_kinematic_vectors,
            [[13, 0, 0, -7084.5, vec_px[i], vec_py[j], 100]],
            axis=0,
        )
...
muon_kinematic_vectors = np.append(
    muon_kinematic_vectors, muon_kinematic_vectors, axis=0
)
...
""" Save to ROOT file. """
muGAN.save_to_ROOT(
    data=muon_kinematic_vectors, filename="repeat_muons/repeat_muons_test1.root"
)
